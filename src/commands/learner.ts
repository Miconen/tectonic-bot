...
class Learner {
    ...
    async half(
        ...
        let addedPoints = await pointsHandler(PointRewardsMap.get('learner_half'), interaction.guild!.id);

        // Handle giving of points, returns a string to be sent as a message.
        let response = await givePoints(addedPoints, channel, interaction);
    ...
    async full(
        ...
        let addedPoints = await pointsHandler(PointRewardsMap.get('learner_full'), interaction.guild!.id);
