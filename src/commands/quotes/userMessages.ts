...
class UserMessages {
    ...
    async steve(interaction: CommandInteraction) {
        await interaction.reply(
            `"Hello I'm Steve I'm a doctor of edging" -Steve`
        )
    }
    @Slash({ name: "swiim", description: "Swiim" })
    async swiim(interaction: CommandInteraction) {
        await interaction.reply(`
        https://media.tenor.com/fyIlma4WWBIAAAAd/maquina-de-lavar-washing-machine.gif
        `)
    }
}
