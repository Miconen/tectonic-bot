...
function parseTime(timeString: string): ParseTimeResult | ParseTimeErrorObject {
    ...
    if (timeParts.length < 2 || timeParts.length > 3) {
        errors.push(
            "Invalid time format. Expected formats: H:M:S, M:S.Ms or M:S"
        )
    ...
    if (timeParts.length === 3 && timeParts[2].includes(".")) {
        errors.push(
            "Invalid time format. Hours and milliseconds cannot coexist."
        )
        // Return early as we rely this not to be the case moving forward
        return { errors }
    }
    ...
    if (hours) {
        ...
        if (isNaN(hours)) {
            errors.push(
                "Invalid time format. Expected a numeric value for hours."
            )
        }
...
export function IsValidTime(option: string) {
    const guard: GuardFunction<CommandInteraction> = async (
        ...
        if (typeof timeString != "string") {
            console.log(
                "↳ Invalid parameter type, expected a string, found: " +
                    timeStringType
            )
            return
        }
